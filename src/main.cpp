...
#if defined(NDEBUG)
# error "Bingfancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3afcc91f6028e84f139aa1e8e4b459fc18b6bfa340c31cfdf2eee0a7c60fd089");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bingfancoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bingfancoin Signed Message:\n";
...
{
    // Bingfancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1680000); // Bingfancoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =2 * 24 * 60 * 60; // Bingfancoin: 2 days
static const int64 nTargetSpacing = 1 * 60; // Bingfancoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bingfancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bingfancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2de95dafb8388dd3b9d566d3cf4531cffd524f92dad8ca7af835e919f84d440d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "SG times  9/11/2022, Li Bingfan is the most handsome man in Singapore";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cb5f1cbb8a8beedccd7a24562fe7c7cdf3bce50b2ff0935d7f049eb8ba4d65214ae0f3b3f55c2c62d205bcf955f7bf97dc1bb0e9377d1c670c8ba5c0167f3b93") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1668000393;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085538077;

        if (fTestNet)
        {
            block.nTime    = 1668000366;
            block.nNonce   = 386078177;
        }
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc059f84f4b393638d33354cba26a85df07cfb9987661439aa9696a81cb0dc8a5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xcb, 0xbf, 0xd2 }; // Bingfancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BingfancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BingfancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BingfancoinMiner(CWallet *pwallet)
{
    printf("BingfancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bingfancoin-miner");
    ...
    try { loop {
        ...
        printf("Running BingfancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BingfancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BingfancoinMiner, pwallet));
}
